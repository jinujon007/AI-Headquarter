...
const navItems = [
  ...
  { href: "/office", label: "3D Office", icon: Gamepad2, highlight: true },
...
export function Sidebar() {
  ...
  // WebSocket hook for active task count
  const { connected, tasks } = useOfficeWs();
  ...
  // Update active task count based on WebSocket tasks
  useEffect(() => {
    const active = tasks.filter(t => t.status === "created").length;
                ...
                <li key={item.href}>
                  ...
                  >
                    ...
                    {item.href === "/tasks" && activeTaskCount > 0 && (
                      <span className="ml-2 flex h-2.5 w-2.5 items-center justify-center rounded-full bg-red-500 text-xs text-white">
                        {activeTaskCount}
                      </span>
                    )}
                  </Link>
